...
# Helper to render one panel (matching the screenshot’s 2-column layout)
        ...
        if finding:
            ...
            if pfx_text:
                ...
                st_html(
                    ...
                    <script>
                      (function(){{
                        const btn = document.getElementById('copy-pfx-btn-{idx}');
                        const txt = {js_text};
                        if (btn) {{
...
with btn_cols[1]:
    if st.button("↺ Reset", use_container_width=True):
        ...
        keys_to_clear = [k for k in st.session_state.keys() if k.startswith("wf_") or k.startswith("finding_")]
